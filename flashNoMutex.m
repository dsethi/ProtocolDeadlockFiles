...
env_o & Sta.Dir.Dirty & Sta.WbMsg.Cmd != WB_Wb & Sta.ShWbMsg.Cmd != SHWB_ShWb &
...
env_o & Sta.Dir.Pending & !Sta.Dir.Local &
...
env_o & Sta.UniMsg[src].Cmd = UNI_Get &
...
env_o & dst != Home &
  ...
  Sta.Dir.Pending & !Sta.Dir.Local &
  Sta.PendReqSrc = Other & Sta.FwdCmd = UNI_Get  -- by Lemma_2
--env_o & dst != Home & Sta.Proc[dst].CacheState != CACHE_E
==>
var
  NxtSta : STATE;
begin
...
begin
  ...
  if Sta.Dir.Dirty then
    ...
    undefine NxtSta.Proc[Home].CacheData;
          ...
          Sta.Dir.HeadPtr = Other
          & forall p : NODE do true -> !Sta.Dir.ShrSet[p] end
          ----& nondet0 COMMENTED BY DJ
         then
    NxtSta.Dir.Local := false;
    NxtSta.Dir.Dirty := true;
    NxtSta.Dir.HeadVld := true;
...
env_o & Sta.UniMsg[src].Cmd = UNI_GetX &
  Sta.UniMsg[src].Proc = Other &
  Sta.Dir.Pending & !Sta.Dir.Local &
...
rule "absNI_InvAck"
env_o & Sta.Dir.Pending & Sta.Collecting &
...
--env_o & Other != Home & Sta.Dir.Pending
==>
var
  NxtSta : STATE;
begin
...
begin
  ...
  if exists p : NODE do p != src & Sta.Dir.InvSet[p] end 
   --| nondet0 COMMENTED BY DJ
  then
...
--NOTE: the antecedents of these progress invariants add up to one and these are right. But they are in a different form
--compared to the invariants in flashabs (presented in paper). Those invariants are incompatible with these.
